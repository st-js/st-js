...
public class RhinoNodeVisitorSupport {
	...
	private static interface Caller {
		<T> void call(AstNode node, AstVisitor<T> visitor, T param);
	}

	static {
		addCaller(Name.class, new Caller() {
			@Override
			public <T> void call(AstNode node, AstVisitor<T> visitor, T param) {
	...
	public <T> void accept(AstNode node, AstVisitor<T> visitor, T param) {
