...
public final class TreeUtils {
    ...
    private final static Set<Tree.Kind> typeTreeKinds = EnumSet.of(Tree.Kind.PRIMITIVE_TYPE, Tree.Kind.PARAMETERIZED_TYPE,
            Tree.Kind.TYPE_PARAMETER, Tree.Kind.ARRAY_TYPE, Tree.Kind.UNBOUNDED_WILDCARD, Tree.Kind.EXTENDS_WILDCARD, Tree.Kind.SUPER_WILDCARD, Tree.Kind.UNION_TYPE);
    ...
    public static List<Element> getTypesFromTree(Tree node) {
        List<Element> typeList = new ArrayList();
        if (node instanceof UnionTypeTree) {
            UnionTypeTree unionTypeTree = (UnionTypeTree) node;
            for (Tree tree : unionTypeTree.getTypeAlternatives()) {
                typeList.addAll(getTypesFromTree(tree));
            }
        } else if (isExpressionTree(node)) {
            Element element = elementFromUse((ExpressionTree) node);
