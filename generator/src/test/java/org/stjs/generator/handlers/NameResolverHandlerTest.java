...
public class NameResolverHandlerTest {
	...
	public void shouldQualifyStaticMememberAccess() throws Exception {
		...
		handlerTester(NameResolverHandler.class, false).assertGenerateString(
				"MyClass.MY_CONSTANT",
				node,
				context().withIdentifier(node,
						new QualifiedName<IdentifierName>(scope, true, NameTypes.CLASS, scope.getDeclaredTypeName())));
	}
	...
	public void shouldQualifyStaticMethodAccess() throws Exception {
		...
		handlerTester(NameResolverHandler.class, false).assertGenerateString(
				"MyClassContainingStaticMethod.myStaticMethod()",
				node,
				context().withMethod(
						node,
						new QualifiedName<MethodName>(scope, true, NameTypes.METHOD, new JavaTypeName(
								withClassName("MyClassContainingStaticMethod")))));
	}
	...
	public void shouldDelegateToDefaultBehaviorIfQNameIsNotFound() throws Exception {
		...
		handlerTester(NameResolverHandler.class, false).assertGenerateString("MyClas.myField.myMethod()", node,
				context());
	}
